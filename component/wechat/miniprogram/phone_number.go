...
// GetPhoneNumberRequest @Description: 获取手机号请求
type GetPhoneNumberRequest struct {
	Code string `json:"code"` //手机号获取凭证
}

// PhoneInfo @Description: 手机号信息
...
// GetPhoneNumber @Description: code换取用户手机号。 每个code只能使用一次，code的有效期为5min
func (business *Business) GetPhoneNumber(in *GetPhoneNumberRequest) (info PhoneInfo, err error) {
	...
	response, err := utils.PostJSON(uri, in)
	if err != nil {
		return
	}
	// 使用通用方法返回错误
