...
func (auth *Auth) Code2SessionContext(ctx context.Context, jsCode string) (result ResCode2Session, err error) {
	var response []byte
	if response, err = utils.HTTPGetContext(ctx, fmt.Sprintf(code2SessionURL, auth.AppID, auth.AppSecret, jsCode)); err != nil {
		return
	}
...
func (auth *Auth) CheckEncryptedDataContext(ctx context.Context, encryptedMsgHash string) (result RspCheckEncryptedData, err error) {
	...
	if response, err = utils.HTTPPostContext(ctx, fmt.Sprintf(checkEncryptedDataURL, at), "encrypted_msg_hash="+encryptedMsgHash); err != nil {
		return
	}
