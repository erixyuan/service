	...
	if m == "GET" || m == "HEAD" {
		if len(params) > 0 {
			if strings.Contains(url, "?") {
				url = url + "&" + HandlerSpace(params)
			} else {
				url = url + "?" + HandlerSpace(params)
			}
		}
	} else {
		body = strings.NewReader(HandlerSpace(params))
	}
	return http.NewRequest(m, url, body)
}

func (cli *Client) NotifyVerify(partnerId, notifyId string) bool {
	var param = url.Values{}
	param.Add("service", "notify_verify")
	param.Add("partner", partnerId)
	param.Add("notify_id", notifyId)
	req, err := NewRequest("GET", cli.notifyVerifyDomain, param)
	if err != nil {
		return false
	}

	rep, err := cli.Client.Do(req)
	if err != nil {
		return false
	}
...
func (cli *Client) GetTradeNotification(req *http.Request) (noti *TradeNotification, err error) {
	if req == nil {
	...
	ok, err := cli.VerifySign(req.Form)
	if ok == false {
		return nil, err
	}
	return noti, err
...
func (cli *Client) AckNotification(w http.ResponseWriter) {
	AckNotification(w)
}
